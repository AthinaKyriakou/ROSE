...
# This file holds a few functions that support the explanation part of ProtoMF
...
def get_top_k_items(item_weights: np.ndarray, items_info: pd.DataFrame, proto_idx: int,
    ...
    """
    This function takes an item-prototype or user-prototype and returns items that represent this prototype.

    For user prototypes, the item_weights matrix contains the list of item embeddings, which means for every
    user prototype we can obtain the items that have the highest embedding values and hence represent the
    prototypes best.
    Note: by using the embedding values and then inspecting the values per user prototype, this is equal to
    creating recommendations for a ficticious user which is maximally close to a specific user prototype
    and maximally distant from all the others

    For item prototypes, the item_weights matrix simply represents the item to item-prototype similarity matrix,
    which means, we can similarly obtain the k items that are closest to these item prototypes.
    
    Parameters
    ----------
    item_weights: numpy array (2D)
        Contains either item embeddings or item to item-prototype similarity measures. Shape is (n_items, n_prototypes)
    items_info: Dataframe
        Contains the item_id field used to look up the item information
    proto_idx: int
        Index of the prototype
    top_k: int, optional, default: 10
        Number of items to return for the prototype, default to 10
    invert: boolean, optional, default=False
        Whether to look for the farthest items instead of closest, default to false
    
    Returns
    -------
    item_infos_top_k: Dataframe
        Contains the top-k closest items to the prototype along with an item weight field.
    """
    # Assure that the prototype is within the available prototypes (index not out of range)
    assert proto_idx < item_weights.shape[1], \
        f'proto_idx {proto_idx} is too high compared to the number of available prototype'

    # For the given prototype, extract all the weights that items return for this prototype
    weights_proto = item_weights[:, proto_idx]

    # Sort the weights of the items and save the ones that belong to the top k (these k items are closest to the given prototype)
    top_k_indexes = np.argsort(weights_proto if invert else -weights_proto)[:top_k]
    top_k_weights = weights_proto[top_k_indexes]

    # We take the dataframe containing the items data, set their ID's as index and filter to only obtain the data for the k closest items
    item_infos_top_k = items_info.set_index('item_id').loc[top_k_indexes]
    # Add a new column and store the weights of the nearest items
    item_infos_top_k['item weight'] = top_k_weights
    # Finally return this dataframe
