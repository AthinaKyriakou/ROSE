...
class ProtoRecDataset(data.Dataset):
    ...
    def __init__(self, data_set, split_set: str, n_neg: int, neg_strategy: str = 'uniform', train_csr = None):
        """
        ...
        print(f'Loading {split_set} data')
        ...
        self.n_users = data_set.num_users
        self.n_items = data_set.num_items

        self.item_ids = data_set.item_ids

        self.coo_matrix = sp.coo_matrix(data_set.csr_matrix)
        self.csr_matrix = data_set.csr_matrix
        #if split_set in ['val', 'test'] and train_csr != None:
            #self.csr_matrix += train_csr
        item_popularity = np.array(data_set.csr_matrix.sum(axis=0)).flatten()
