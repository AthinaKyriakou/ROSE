...
class Metric_Exp_PSPNFNS(Metric_Exp):
    ...
    name: str, optional, default: 'Metric_Exp_PSPNFNS'

    References
    ----------
    [1] Tan, J., Xu, S., Ge, Y., Li, Y., Chen, X., &amp; Zhang, Y. (2021). Counterfactual explainable recommendation. Proceedings of the 30th ACM International Conference on Information &amp;amp; Knowledge Management. https://doi.org/10.1145/3459637.3482420
    """

    def __init__(self, name="Metric_Exp_PSPNFNS", rec_k=10, feature_k=10):
        Metric_Exp.__init__(self, name=name, rec_k=rec_k, feature_k=feature_k)

    def compute(self, model, explainer, explanations):
        """The main function to compute PS PN FNS
        ...
        if self.explainer.name == "Exp_LIMERS":
            ...
            if not self.model.train_set.item_features.empty:
                self.item_f = True
                self.item_features_original = copy.deepcopy(
                    self.model.train_set.item_features
                )
            if not self.model.train_set.user_features.empty:
                self.user_f = True
                self.user_features_original = copy.deepcopy(
                    self.model.train_set.user_features
                )
        ...
        with tqdm(total=total, desc="Re-evaluate after features removal... ") as pbar:
            for i, (user, item, exp) in enumerate(self.explanations):
                ...
                if self.explainer.name == "Exp_LIMERS":
                    ...
                    raise ValueError(
                        f"Metric {self.name} does not support {self.explainer.name}"
                    )
        ...
        if len(self.explanations) != 0:
            ...
            if (pn + ps) != 0:
                ...
                fns_dist = [
                    (
                        2 * pn_dist[i] * ps_dist[i] / (pn_dist[i] + ps_dist[i])
                        if (pn_dist[i] + ps_dist[i]) > 0
                        else 0
                    )
                    for i, _ in enumerate(pn_dist)
                ]
            else:
                fns = 0
        else:
    ...
    def _reevaluate_limers(self, exp, user, pn=True):
        ...
        if pn:
            if self.item_f == True and self.user_f == True:
                ...
                self.model.train_set.item_features = self.item_features_original[
                    ~self.item_features_original["feature"].isin(item_exp)
                ]
                self.model.train_set.user_features = self.user_features_original[
                    ~self.user_features_original["feature"].isin(user_exp)
                ]
            elif self.item_f == True:
                self.model.train_set.item_features = self.item_features_original[
                    ~self.item_features_original["feature"].isin(exp)
                ]
            else:
                self.model.train_set.user_features = self.user_features_original[
                    ~self.user_features_original["feature"].isin(exp)
                ]
        ...
        rank_scores = self.model.recommend_to_multiple_users([user], self.rec_k)[
            "item_id"
        ].to_list()
